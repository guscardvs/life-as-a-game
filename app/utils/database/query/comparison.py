...
from .interface import BindClause, Comparator, Comparison, FieldType, Sortable


def always_true(field: FieldType, target: Any) -> Comparison:
    del field, target
    return sa.true()


def equals(field: FieldType, target: Any) -> Comparison:
    return field == target


def not_equals(field: FieldType, target: Any) -> Comparison:
    return field != target
...
def insensitive_like(
    opt: Literal["like", "rlike", "llike"] = "like",
) -> Comparator[str]:
...
def includes(field: FieldType, target: Collection[Any]) -> Comparison:
    return field.in_(list(target))


def excludes(field: FieldType, target: Collection[Any]) -> Comparison:
    return field.not_in(list(target))


def json_contains(field: FieldType, target: Any) -> Comparison:
    return sa.func.json_contains(field, f'"{target}"')


def json_empty(field: FieldType, target: Any) -> Comparison:
    func_length = sa.func.json_length(field)
    return func_length == 0 if target else func_length != 0


def make_relation_check(clause: BindClause) -> Comparator[Any]:
